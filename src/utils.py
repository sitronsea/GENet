...
def shift(a, axis, orientation, dims, batch_tensor=False):
    """
    For a tensor, whose grid axes are flattened, perform
    a shift along a certain axis
    """
    if batch_tensor:
        original_shape = tuple(a.shape)
        new_shape = (original_shape[0], *dims, *tuple(a.shape[2:]))
        ...
        original_shape = tuple(a.shape)
        new_shape = (*dims, *tuple(a.shape[1:]))
